...
class Regions(SageObject):
    ...
    def get_regions(self,B):
        ...
        if(B>self._N):
            F=self._F
            eps=self._eps
            newelts=dict([(IntegerRing()(n),[I.gens_reduced()[0] for I in v if I.is_principal()]) for n,v in self._F.ideals_of_bdd_norm(B-1).items() if n>=self._N])
            self._N=B
            for nn in range(len(self._epsto),B):
                self._epsto.append(self._epsto[nn-1]*eps)
            for nn,v in newelts.items():
                assert nn not in regions
                regions[nn]=[]
                for q in v:
                    invden=1/q
            ...
            pass
        return [reg for nn,lst in regions.items() if nn<B for reg in lst]
...
class QuadraticContinuedFraction(SageObject):
    ...
    def rangea_gen(self,xmin,ymin,xmax,ymax):
        ...
        def rangea(t):
            tp0=t*wprime0
            tp1=t*wprime1
            return sorted(list(set(range(floor(xmin-1-tp0-wprime0),ceil(xmax-tp0)+1)).union(set(range(floor(ymin-1-tp1-wprime1),ceil(ymax-tp1)+1)))),key=abs)
        return rangea
